# From project euler 107 
# Minimum spanning tree
# DataStructure : Graph
# Technique : Greedy, Heaps ?

from collections import defaultdict

def edge(G,u,v,w): # Graph connected weighted undirected graph
    """ Graph(G) is of type defaultdict(dict) """
    G[u][v] = w
    G[v][u] = w

def addvertex(SG, G):
    """
    Chooses nearest vertex to spanning tree(SG) and adds it to SG from main graph(G).
    """
    minw = float('inf')
    minu = None
    minv = None
    for u in SG.keys(): # From each vertex in spanning tree
            for v in G[u].keys(): # iterate in their adjacency list
                if not SG.has_key(v): # to find new vertex to be added to spanning tree
                    if G[u][v] < minw: # which has minimun weight
                        minu, minv = u, v
                        minw = G[u][v]
    if minv: # add new vertex
        edge(SG,minu,minv,minw)
        return True
    else:
        return False

def prims_minimum_spanning_tree(G):
    """Returns Spanning tree of G"""
    SG = defaultdict(dict) # spanning graph
    # Select some random vertex, here we select first vertex
    v = G.keys()[0]
    SG[v] = {} # Initialization
    while addvertex(SG, G): pass
    return SG
...
"""
    ...
    G = defaultdict(dict)
    costG = 0
    Graph = (g for g in Graph.split('\n') if g.strip() != "")
    for i,g in enumerate(Graph):
        for j,e in enumerate(g.split(',')):
            if e != '-' and j > i:
                ie = int(e)
                costG += ie
                edge(G,i,j,ie)

    prims = prims_minimum_spanning_tree(G)
    costP = 0
    for k,v in prims.items():
        costP += sum(w for u,w in v.items() if u > k)
    print costG-costP
